...
use crate::{
    ...
    il2cpp::{hook::UnityEngine_InputLegacyModule, symbols::Thread, types::Vector2_t},
    windows::wnd_hook,
};
...
extern "C" fn IDXGISwapChain_Present(
    ...
    for (callback, userdata) in callbacks.iter() {
        ...
        unsafe {
            callback(this, *userdata as *mut c_void);
        }
    }
            ...
            if let egui::ViewportCommand::IMERect(rect) = cmd {
                ...
                *IME_COMPOSITION_POS.lock().unwrap() = (x, y_unity);
                ...
                Thread::main_thread().schedule(|| {
                    ...
                    UnityEngine_InputLegacyModule::Input::set_compositionCursorPos(Vector2_t {
                        x,
                        y: y_unity,
                    });
                });
