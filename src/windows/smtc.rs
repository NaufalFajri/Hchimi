...
use windows::{
    ...
    Win32::{
        ...
        UI::Shell::{FOLDERID_Programs, SHGetKnownFolderPath, KF_FLAG_DEFAULT},
    },
};
...
use crate::{
    ...
    il2cpp::{
        ...
        hook::{
            umamusume::{
                AssetManager,
                Director::{self, LiveLoadSettings},
                HomeViewController, HubViewControllerBase, JukeboxBgmSelector, JukeboxHomeTopUI,
                LiveViewController, MasterJukeboxSetlistMusicData, SceneManager as UmaSceneManager,
                TempData::JukeboxSetListPlayingData,
                WorkDataManager, WorkJukeboxData,
            },
            UnityEngine_AssetBundleModule::AssetBundle,
            UnityEngine_CoreModule::{
                Behaviour, Graphics, RenderTexture, Scene, SceneManager, Texture, Texture2D,
            },
...
static SMTC_INSTANCE: Lazy<Mutex<Option<SystemMediaTransportControls>>> =
...
unsafe fn create_shortcut(name: &str) {
    if let Ok(shell_link) = CoCreateInstance::<_, windows::Win32::UI::Shell::IShellLinkW>(
        ...
        exe_path_u16.push(0);
        shell_link
            .SetPath(PCWSTR::from_raw(exe_path_u16.as_ptr()))
            .unwrap();
        ...
        work_dir_u16.push(0);
        shell_link
            .SetWorkingDirectory(PCWSTR::from_raw(work_dir_u16.as_ptr()))
            .unwrap();

        if let Ok(property_store) =
            shell_link.cast::<windows::Win32::UI::Shell::PropertiesSystem::IPropertyStore>()
        {
            ...
            let mut pv = std::mem::ManuallyDrop::new(
            ...
            unsafe {
                ...
                if !alloc_ptr.is_null() {
                    ...
                    let _ =
                        windows::Win32::System::Com::StructuredStorage::PropVariantClear(&mut *pv);
                }
            }
        }
        ...
        if let Ok(persist_file) = shell_link.cast::<IPersistFile>() {
            if let Ok(pwstr) = SHGetKnownFolderPath(&FOLDERID_Programs, KF_FLAG_DEFAULT, None) {
                ...
                wide_path.push(0);
                persist_file
                    .Save(PCWSTR::from_raw(wide_path.as_ptr()), true)
                    .unwrap();
            }
        }
    }
...
pub fn on_update() {
    ...
    let smtc = if let Some(s) = smtc_guard.as_ref() {
        s
    } else {
        return;
    };
    ...
    if scene.handle != current_scene_handle {
        unsafe {
            CURRENT_SCENE_HANDLE = scene.handle;
        }
    ...
    let should_update =
        unsafe { LAST_UPDATE.map_or(true, |last| now.duration_since(last).as_millis() >= 1000) };
...
fn get_jacket_texture(music_id: i32) -> Option<*mut Il2CppObject> {
    ...
    if t2d_type_obj.is_null() {
        return None;
    }

    let tex =
        AssetBundle::LoadAsset_Internal_orig(bundle, jacket_name.to_il2cpp_string(), t2d_type_obj);
    if tex.is_null() {
        return None;
    }
...
fn generate_thumbnail_and_update(
...
fn get_temp_set_list_playing_data(top_ui: *mut Il2CppObject) -> *mut Il2CppObject {
    if get_method_cached(
        unsafe { (*top_ui).klass() },
        c"get_TempSetListPlayingData",
        0,
    )
    .is_ok()
    {
...
fn update_metadata_home(smtc: &SystemMediaTransportControls) {
    ...
    if !hub_vc.is_null() {
        let hub_name =
            unsafe { std::ffi::CStr::from_ptr((*(*hub_vc).klass()).name) }.to_string_lossy();
                ...
                if !data.is_null() {
                    ...
                    if is_playing {
                        ...
                        if !music_data.is_null() {
                            let music_id =
                                MasterJukeboxSetlistMusicData::JukeboxSetlistMusicData::get_MusicId(
                                    music_data,
                                );
                            if music_id != 0 {
                                unsafe {
                                    LAST_HOME_MUSIC_ID = music_id;
                                }
                                update_metadata(smtc, music_id);
                            }
                        }
...
fn get_current_hub_view_child_controller() -> *mut Il2CppObject {
    ...
    let vc = get_current_view_controller();
    debug!(
        "[smtc] get_current_hub_view_child_controller: vc = {:?}",
        vc
    );
    if vc.is_null() {
        return std::ptr::null_mut();
...
fn handle_button_pressed(args: &SystemMediaTransportControlsButtonPressedEventArgs) {
    if let Ok(button) = args.Button() {
        ...
        Thread::main_thread().schedule(move || {
            ...
            if !vc.is_null() {
                let name =
                    unsafe { std::ffi::CStr::from_ptr((*(*vc).klass()).name) }.to_string_lossy();
                if name == "LiveViewController" {
            ...
            if !hub_vc.is_null() {
                let hub_name = unsafe { std::ffi::CStr::from_ptr((*(*hub_vc).klass()).name) }
                    .to_string_lossy();
                debug!("[smtc] hub_vc name = {}", hub_name);
                if hub_name == "HomeViewController" {
                        ...
                        if button == SystemMediaTransportControlsButton::Play {
                            ...
                            if !data.is_null() {
                                ...
                                if set_list_id > 0 {
                                    ...
                                    if !selector.is_null() {
                                        JukeboxBgmSelector::PlayCoroutinePlaySetList(
                                            selector, true, 0.0, true,
                                        );
                                    }
                                }
                            }
                            if !is_set_list {
                                ...
                                if get_method_cached(JukeboxHomeTopUI_class, c"SetPlayMusicFlag", 1)
                                    .is_ok()
                                {
