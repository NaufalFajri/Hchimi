...
extern "C" fn dlopen(filename: *const c_char, flags: c_int) -> *mut c_void {
    ...
    let orig_fn: DlopenFn =
        unsafe { std::mem::transmute(hachimi.interceptor.get_trampoline_addr(dlopen as usize)) };
...
extern "C" fn JNINativeInterface_RegisterNatives(
    ...
    for method in methods {
        ...
        if name == "nativeInjectEvent" {
            ...
            hachimi
                .interceptor
                .unhook(JNINativeInterface_RegisterNatives as usize);
        }
    }
...
fn init_internal(env: *mut jni::sys::JNIEnv) -> Result<(), Error> {
    ...
    if !hachimi.config.load().disable_gui {
        ...
        hachimi.interceptor.hook(
            register_natives_addr,
            JNINativeInterface_RegisterNatives as usize,
        )?;
    }
