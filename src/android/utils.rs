...
pub fn set_keyboard_visible(visible: bool) {
    ...
    let result = (|| -> jni::errors::Result<()> {
        let activity =
            get_activity(unsafe { env.unsafe_clone() }).ok_or(jni::errors::Error::JavaException)?;
...
pub fn set_audio_capture_policy_all() {
    ...
    let result = (|| -> jni::errors::Result<()> {
        ...
        if api_level < 29 {
            info!(
                "setAllowedCapturePolicy ignored: API level {} is below 29",
                api_level
            );
            return Ok(());
        }
...
pub fn get_available_disk_space() -> Option<u64> {
    ...
    let result = (|| -> jni::errors::Result<u64> {
        ...
        // activity.getDataDir()
        let data_dir = env
            .call_method(&activity, "getDataDir", "()Ljava/io/File;", &[])?
            .l()?;

        // dataDir.getFreeSpace()
        let free_bytes = env
            .call_method(&data_dir, "getFreeSpace", "()J", &[])?
            .j()?;
