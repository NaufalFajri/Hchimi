...
use crate::{
    core::{ext::Utf16StringExt, game::Region, hachimi::AssetMetadata, Hachimi},
    il2cpp::{
        api::il2cpp_resolve_icall,
        ext::{Il2CppObjectExt, Il2CppStringExt},
        hook::{
            umamusume::{
                StoryParamChangeEffect, StoryRaceTextAsset, StoryTimelineData, TextDotData,
                TextRubyData,
            },
            Cute_UI_Assembly::AtlasReference,
            UnityEngine_CoreModule::{GameObject, Object, Texture2D},
        },
        symbols::GCHandle,
        types::*,
    },
...
pub static REQUEST_INFOS: Lazy<Mutex<FnvHashMap<usize, RequestInfo>>> =
    ...
    if let Some(meta_bundle_name) = &metadata.bundle_name {
        ...
        if Hachimi::instance().game.region == Region::Japan {
            if !name_ptr.is_null() {
                ...
                if let Some(real_hash) =
                    crate::il2cpp::sql::MetaData::get_hash(&logical_name.to_string())
                {
