...
use crate::{
    ...
    il2cpp::{
        ...
        symbols::{get_assembly_image, get_class, get_method_addr, Array},
        types::*,
    },
};
...
extern "C" fn RegisterDownloadForLiveDirector(register: *mut Il2CppObject, id: i32) {
    get_orig_fn!(
        RegisterDownloadForLiveDirector,
        RegisterDownloadForLiveDirectorFn
    )(register, id);
    ...
    let paths = [
        format!(
            "Live/Image/Champions/tex_championslive_racename_{:02}",
            resource_id
        ),
        format!(
            "Live/Image/Champions/tex_championslive_year_{:02}",
            texture_year_idx
        ),
        format!(
            "Live/Image/Champions/tex_championslive_year_l_{:02}",
            texture_year_idx
        ),
    ];

    let path_array = Array::<*mut Il2CppString>::new(string_class, paths.len());
    if path_array.this.is_null() {
        return;
    }
