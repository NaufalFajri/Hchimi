use super::{
    HorseRaceInfo, Jikkyo, JikkyoControllerBase, RaceEventPlayer, RaceHorseManagerBase,
    RaceHorseManagerReplay, RaceMainViewController, RaceManager, RaceSimulateData,
    RaceSimulateEventData, RaceSimulateReader, RaceUI, RaceUIMiniMap, RaceViewReplay,
    SimulateEventType, SkillManager,
};
use crate::{
    core::{
        game::Region,
        gui,
        utils::{clear_il2cpp_list, race_seek_seh, race_seek_stage},
        Hachimi,
    },
    il2cpp::{
        api::{il2cpp_class_get_method_from_name, il2cpp_object_get_class},
...
pub fn seek_sync(target_time: f32) -> bool {
    ...
    let prev_time = RaceSimulateReader::replay_cur_time(race_manager).unwrap_or(f32::from_bits(
        gui::RACE_SLIDER_LAST_APPLIED.load(Ordering::Acquire),
    ));
...
fn resync_used_skills(race_manager: *mut Il2CppObject, target_time: f32) {
    ...
    for event in event_list.iter() {
        ...
        if RaceSimulateEventData::get_type(event) != SimulateEventType::Skill {
            continue;
        }
        if RaceSimulateEventData::get_frameTime(event) > target_time {
            continue;
        }
...
pub fn toggle_playback() {
    ...
    if playback_gated(race_manager) {
        return;
    }

    if is_paused() {
        unsafe {
            let class = il2cpp_object_get_class(race_manager);
            let method = il2cpp_class_get_method_from_name(class, c"ResumeRace".as_ptr(), 0);
            if !method.is_null() {
                let func: extern "C" fn(*mut Il2CppObject) =
                    std::mem::transmute((*method).methodPointer);
                func(race_manager);
            } else {
        ...
        unsafe {
            let class = il2cpp_object_get_class(race_manager);
            let method = il2cpp_class_get_method_from_name(class, c"PauseRace".as_ptr(), 0);
            if !method.is_null() {
                let func: extern "C" fn(*mut Il2CppObject) =
                    std::mem::transmute((*method).methodPointer);
                func(race_manager);
...
pub fn is_paused() -> bool {
    ...
    unsafe {
        let class = il2cpp_object_get_class(race_manager);
        let method = il2cpp_class_get_method_from_name(class, c"IsPaused".as_ptr(), 0);
        if !method.is_null() {
            let func: extern "C" fn(*mut Il2CppObject) -> bool =
                std::mem::transmute((*method).methodPointer);
            return func(race_manager);
        }
    }

    IsPaused(race_manager)
}
...
pub fn init(umamusume: *const Il2CppImage) {
    ...
    unsafe {
        ...
        UPDATE_CAMERA_EVENT_ALL_ADDR =
            get_method_addr(RaceManagerReplayBase, c"UpdateCameraEventAll", 1);
        LATE_UPDATE_CAMERA_EVENT_ALL_ADDR =
            get_method_addr(RaceManagerReplayBase, c"LateUpdateCameraEventAll", 1);
        IS_CUT_IN_PLAYING_OR_RESERVED_ADDR =
            get_method_addr(RaceManagerReplayBase, c"IsCutInPlayingOrReserved", 0);
