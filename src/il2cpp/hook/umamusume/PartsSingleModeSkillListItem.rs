...
use crate::{
    core::{game::Region, utils::str_visual_len, Hachimi},
    il2cpp::{
        api::il2cpp_class_from_il2cpp_type,
        ext::{Il2CppStringExt, StringExt},
        hook::{
            umamusume::{GallopUtil::without_text_wrap, TextCommon},
...
extern "C" fn UpdateItemJp(
    ...
    let effective_on_click = if Hachimi::instance().config.load().skill_info_dialog
        && !skill_info.is_null()
    {
...
extern "C" fn on_click_skill_button(_ptr: usize, skill_id: i32) {
    ...
    let skill_name = to_s(TextDataQuery::get_skill_name(skill_id))
        .unwrap_or_else(|| to_s(Some(MasterDataUtil::GetSkillName(skill_id))).unwrap());
    let skill_desc = to_s(TextDataQuery::get_skill_desc(skill_id)).unwrap_or_else(|| {
        to_s(Some(
            Hachimi::instance()
                .skill_info
                .load()
...
type UpdateItemGlobalTwFn = extern "C" fn(
    this: *mut Il2CppObject,
    skill_info: *mut Il2CppObject,
    is_plate_effect_enable: bool,
    resource_hash: i32,
);
extern "C" fn UpdateItemGlobalTw(
    this: *mut Il2CppObject,
    skill_info: *mut Il2CppObject,
    is_plate_effect_enable: bool,
    resource_hash: i32,
    ...
    UpdateItemCommon(this, skill_info, || {
        get_orig_fn!(UpdateItemGlobalTw, UpdateItemGlobalTwFn)(
            this,
            skill_info,
            is_plate_effect_enable,
            resource_hash,
...
pub fn init(umamusume: *const Il2CppImage) {
    ...
    unsafe {
        if Hachimi::instance().game.region == Region::Japan {
            _ONCLICKBUTTON_FIELD =
                get_field_from_name(PartsSingleModeSkillListItem, c"_onClickButton");
        ...
        NEED_SKILL_POINT_ROOT_FIELD =
            get_field_from_name(PartsSingleModeSkillListItem, c"_needSkillPointRoot");
        INFO_FIELD = get_field_from_name(PartsSingleModeSkillListItem, c"_info");
        set_skill_name_text_addr =
            get_method_addr(PartsSingleModeSkillListItem, c"SetSkillNameText", 0);
